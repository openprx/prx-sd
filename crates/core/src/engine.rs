...
impl ScanEngine {
    ...
    pub fn new(config: ScanConfig) -> Result<Self> {
        ...
        let signatures = SignatureDatabase::open(&config.signatures_dir)
            .context("failed to open signature database")?;

        let yara = YaraEngine::load_rules(&config.yara_rules_dir)
            .context("failed to initialise YARA engine")?;
    ...
    pub fn reload_signatures(&mut self) -> Result<()> {
        ...
        let signatures = SignatureDatabase::open(&self.config.signatures_dir)
            .context("failed to reload signature database")?;

        let yara = YaraEngine::load_rules(&self.config.yara_rules_dir)
...
mod tests {
    ...
    fn new_fails_with_nonexistent_yara_dir() {
        ...
        assert!(
            err_msg.contains("YARA"),
            "expected YARA error, got: {err_msg}"
        );
    }
